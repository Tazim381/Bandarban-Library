...
const BookDetailsCard = ({ book }) => {
  ...
  const [token, setToken] = useState(
    localStorage.getItem("set-token-for-user")
  );
  ...
  const handleDelete = (id) => {
    ...
    axios
      .delete(`http://localhost:5000/api/book/deleteBook/${id}`)
      ...
      <Card className="w-full max-w-[70rem] h-3/4 flex-row gap-10">
          ...
          <div className="">
            {book.image ? (
              ...
              <div>
                ...
                <div className="font-bold text-3xl mt-24">{book.bookName}</div>
                <div className="font-semibold text-xl">{book.authorName}</div>
                </div>
              </div>
            )}
          </div>
        ...
        <CardBody className="flex flex-col  justify-center">
          <Typography variant="h6" color="gray" className="mb-4 uppercase">
            <p>
              <span className="font-bold"> Book Name : </span> {book.bookName}
            </p>
          </Typography>
          ...
          <Typography color="gray" className="mb-8 font-normal">
            ...
            <p>
              <span className="font-bold">Book Entry Language : </span>
              {book.entryLanguage}
            </p>
          </Typography>
